...
func (s *SqlGroupStore) Create(group *model.Group) (*model.Group, error) {
	if len(group.Id) != 0 {
		return nil, fmt.Errorf("SqlGroupStore.GroupCreate, model.group.id.app_error groupd id %s", group.Id)
	}
...
func (s *SqlGroupStore) Update(group *model.Group) (*model.Group, error) {
	...
	if group.DeleteAt != retrievedGroup.DeleteAt && group.DeleteAt != 0 {
		return nil, fmt.Errorf("SqlGroupStore.GroupUpdate", "model.group.delete_at.app_error")
	}
...
func (s *SqlGroupStore) CreateGroupSyncable(groupSyncable *model.GroupSyncable) (*model.GroupSyncable, error) {
	...
	default:
		return nil, fmt.Errorf("model.group_syncable.type.app_error, group_id= %s syncable_id=%s", groupSyncable.GroupId, groupSyncable.SyncableId)
	}

	if insertErr != nil {
		return nil, fmt.Errorf("store.insert_error group_id= %s syncable_id=%s", groupSyncable.GroupId, groupSyncable.SyncableId)
	}
...
func (s *SqlGroupStore) GetAllGroupSyncablesByGroupId(groupID string, syncableType model.GroupSyncableType) ([]*model.GroupSyncable, error) {
	...
	errF := func(msg string) error {
		return fmt.Errorf("store.select_error %s", msg)
	}
